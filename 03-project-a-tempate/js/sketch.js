...
function draw() {
  ...
  if (stand) {
    drawStand(); //the warawara is standing
  } else if (inflate) {
    drawInflate();//the warawara is inflated
  } else {
    creature(x, y); //the warawara flies
  }
  
  if (stand && inflate == true) {
      image(img, 250, 250, imgWidth, imgHeight);//fish guts img
  }
 // if (mouseIsPressed) {
...
function mousePressed() {
  ...
  //if mouse is pressed within area of creature
  if (dis < 400 && stand) {
    ifGo = true; //the warawara floats
    stand = false;
